...
def issue_release_seal(*, certificate: Dict[str, Any], attestation_verification: Dict[str, Any], independent_replay: Dict[str, Any], ledger: Dict[str, Any], portal_contract: Dict[str, Any]) -> Dict[str, Any]:
    ...
    replay_output=independent_replay.get("replay_output",{})
    replay_source=replay_output.get("source",{})
    checks={
        "certificate_eligible":certificate.get("eligible_for_human_promotion") is True,
        "attestation_verified":attestation_verification.get("passed") is True,
        "attestation_binds_certificate":attestation_verification.get("certificate_hash")==certificate.get("certificate_hash"),
        "independent_replay_passed":independent_replay.get("passed") is True,
        "replay_subject_matches":replay_output.get("node_id")==certificate.get("subject_node_id"),
        "replay_compilation_matches":replay_output.get("compilation_hash")==certificate.get("compilation_hash"),
        "replay_source_matches":replay_source.get("source_hash")==certificate.get("source_hash"),
        "ledger_valid":ledger_check["passed"],
        "certificate_active":state.get("state")=="ACTIVE",
        "portal_contract_valid":portal_contract.get("passed") is True,
        "no_authority_transfer":certificate.get("authority_transfer") is False and attestation_verification.get("authority_transfer") is False,
    }
